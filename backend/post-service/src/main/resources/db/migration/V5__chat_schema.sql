...
BEGIN
    ...
    SELECT COUNT(*) INTO plural_rooms
    FROM information_schema.tables
    WHERE table_schema = DATABASE() AND table_name = 'chat_rooms';

    SELECT COUNT(*) INTO singular_rooms
    FROM information_schema.tables
    ...
    WHERE table_schema = DATABASE() AND table_name = 'chat_messages'
      AND column_name = 'chat_room_id' AND referenced_table_name = 'chat_rooms';
    IF has_fk = 0 THEN
        ALTER TABLE chat_messages
            ADD CONSTRAINT fk_chat_message_room FOREIGN KEY (chat_room_id) REFERENCES chat_rooms(id);
    END IF;
END$$
DELIMITER ;
