...
public class GeminiClient {
    ...
    public void requireAvailable() {
        if (!enabled || key.isBlank()) {
            throw new AiException(HttpStatus.SERVICE_UNAVAILABLE, "AI_UNAVAILABLE", "AI 작성 도움을 사용할 수 없습니다. 직접 작성해주세요.");
        }
    }
    ...
    public JsonNode generate(String instruction, List<Map<String, Object>> parts, Map<String, Object> schema) {
        ...
        try {
            return http.post().uri("/models/{model}:generateContent", model)
                    ...
                    .body(
                            Map.of("systemInstruction", Map.of("parts", List.of(Map.of("text", instruction))),
                    ...
                    .exchange((request, response) -> {
                        ...
                        if (status == 429 || status == 503) {
                            throw new AiException(HttpStatus.SERVICE_UNAVAILABLE, "PROVIDER_BUSY", "AI 서비스가 혼잡하거나 사용 한도에 도달했습니다. 잠시 후 다시 시도해주세요.");
                        }

                        if (!response.getStatusCode().is2xxSuccessful()) {
                            throw new AiException(HttpStatus.BAD_GATEWAY, "PROVIDER_ERROR", "AI 서비스 설정 또는 연결을 확인해야 합니다. 직접 작성해주세요.");
                        }
            ...
            throw new AiException(HttpStatus.GATEWAY_TIMEOUT, "AI_TIMEOUT", "AI 응답이 지연되고 있습니다. 입력은 유지되므로 직접 작성하거나 다시 시도해주세요.");
        } catch (Exception e) {
            throw AiException.output();
        }
    }
