...
public class PostService {
    ...
    public Long createPost(
        ...
        RegionResponse response =
                postViewerService.requireRegion(authorEmail);
    ...
    public NearbyRepairRequest.Result getNearbyPosts(
        ...
        // ALL은 활동 지역이 없어도 전체 조회 가능.
        // 지역 표시용 정보만 best-effort 조회.
        RegionResponse region = email == null
                ? null
        ...
        Page<NearbyRepairRequest> posts =
        ...
        HashMap<String, String> nicknameCache =
                new HashMap<>();
    ...
    private void guardNoActiveDeal(Long postId) {
        fixDealRepository.findByPostId(postId)
                .ifPresent(deal -> {
                    if (deal.getStatus()
                            != FixDealStatus.COMPLETED
                            && deal.getStatus()
                            != FixDealStatus.CANCELED) {
