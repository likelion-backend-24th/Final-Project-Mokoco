...
public class AiDraftCache {
    ...
    public JsonNode get(String key, Runnable acquireQuota, Supplier<JsonNode> generate) {
        ...
        synchronized (this) {
            ...
            if (entry.result != null) return entry.result.deepCopy();
            if (entry.running) throw limited("AI_IN_PROGRESS", "동일한 입력을 처리 중입니다. 잠시 기다려주세요.");
            if (entry.attempts >= 2) throw limited("AI_RETRY_LIMIT", "같은 입력의 재시도 한도에 도달했습니다. 10분 후 다시 시도하거나 직접 작성해주세요.");
            if (entry.retryAt > now) throw limited("AI_RETRY_WAIT", "실패한 요청은 30초 후 한 번만 다시 시도할 수 있습니다.");
            acquireQuota.run();
            entry.attempts++;
            entry.running = true;
        }
        try {
            ...
            synchronized (this) { entry.result = result.deepCopy(); entry.expires = clock.millis() + TTL; }
            return result.deepCopy();
        } finally {
            synchronized (this) { entry.running = false; entry.retryAt = clock.millis() + RETRY_DELAY; }
        }
    }
