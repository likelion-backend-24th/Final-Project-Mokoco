...
public class AiRateLimit {
    ...
    public synchronized void acquire(Long userId) {
        ...
        if (history.size() >= dayLimit || history.stream().filter(t -> t.isAfter(now.minusSeconds(60))).count() >= minuteLimit) {
            throw limited();
        }
    ...
    private AiException limited() {
        return new AiException(HttpStatus.TOO_MANY_REQUESTS, "AI_LIMIT", "AI 작성 도움의 사용 한도에 도달했습니다. 직접 작성하거나 나중에 다시 시도해주세요.");
    }
}
