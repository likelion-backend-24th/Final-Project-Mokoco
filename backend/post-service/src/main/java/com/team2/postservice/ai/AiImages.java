...
public class AiImages {
    public List<Map<String, Object>> parts(List<MultipartFile> files) {
        if (files == null || files.isEmpty() || files.size() > 5) throw AiException.input("분석할 사진을 1~5장 선택해주세요.");
        ...
        for (var file : files) {
            try (var stream = new MemoryCacheImageInputStream(file.getInputStream())) {
                ...
                try {
                    ...
                    if (!mime.equalsIgnoreCase(file.getContentType())) throw unsupported();
            ...
            catch (IOException | RuntimeException e) { throw unsupported(); }
        }
        return result;
    }
    private AiException unsupported() { return new AiException(HttpStatus.UNSUPPORTED_MEDIA_TYPE, "INVALID_IMAGE", "올바른 JPEG, PNG 또는 WebP 사진을 선택해주세요."); }
}
