...
class AiImagesTest {
    ...
    @Test void convertsAndDownsizesActualPixels() throws Exception {
        Map<String, Object> part = images.parts(
                List.of(new MockMultipartFile("images","photo.png","image/png",
                        png(2000,1000))))
                .getFirst();

        Map<?,?> data = (Map<?,?>) part.get("inlineData");
        assertThat(data.get("mimeType")).isEqualTo("image/jpeg");

        BufferedImage decoded = ImageIO.read(new ByteArrayInputStream(Base64.getDecoder().decode((String)data.get("data"))));
    ...
    @Test void rejectsForgedMimeAndCorruptImage() throws Exception {
        MockMultipartFile forged = new MockMultipartFile("images","file.jpg","image/jpeg",png(10,10));
    ...
    @Test void rejectsEmptySixImagesAndOversizedFile() throws Exception {
        MockMultipartFile file = new MockMultipartFile("images","file.png","image/png",png(10,10));
    ...
    @Test void webpDecoderIsInstalled() {
