...
class InternalPaymentControllerTest {
    @Test void onlyInternalKeyCanReadAuthoritativePaymentContext() throws Exception {
        ...
        Post post = Post.builder().title("repair").content("repair").authorEmail("owner@test").regionName("Seoul").category(PostCategory.ALL).build();
        ReflectionTestUtils.setField(post, "id", 1L);
        Proposal proposal = Proposal.builder().post(post).repairerEmail("repair@test").content("repair").estimatedPrice(15000).build();
        proposal.adopt();
        ...
        mvc.perform(get("/internal/payments/posts/1").header("X-Internal-Service-Key", "test-key"))
                ...
                .andExpect(jsonPath("$.payerId").value(10)).andExpect(jsonPath("$.payeeEmail").value("repair@test"))
