...
class ProposalChatIntegrationTest {
    ...
    @BeforeEach void setup() {
        ...
        new TransactionTemplate(transactions).executeWithoutResult(status -> {
            var post = posts.save(Post.builder().title("수리 요청").content("다리 수리").authorEmail("requester@test.invalid")
    ...
    @Test void adoptionLinksExistingRoomAndPreservesMessages() {
        var room = chatRooms.createForProposal(first,20L);
        var message = chat.send(room.chatRoomId(),10L,"상담 이력");
        proposals.adoptProposal(postId,first,"requester@test.invalid");
        var adopted = chatRooms.createForProposal(first,10L);
    ...
    @Test void simultaneousCreationReturnsOneRoom() throws Exception {
        ...
        try (var pool = Executors.newFixedThreadPool(2)) {
            var a = pool.submit(() -> { start.await(); return chatRooms.createForProposal(first,10L); });
            var b = pool.submit(() -> { start.await(); return chatRooms.createForProposal(first,20L); });
            start.countDown();
    ...
    @Test void simultaneousAdoptionAndCreationUseSameRoom() throws Exception {
        var start = new CountDownLatch(1);
        try (var pool = Executors.newFixedThreadPool(2)) {
            var a = pool.submit(() -> { start.await(); return chatRooms.createForProposal(first,20L); });
            var b = pool.submit(() -> { start.await(); proposals.adoptProposal(postId,first,"requester@test.invalid"); return true; });
            start.countDown();
            var created = a.get(10,TimeUnit.SECONDS); b.get(10,TimeUnit.SECONDS);
            var linked = chatRooms.getForProposal(first,10L);
    ...
    @Test void legacyDealRoomIsReusedByProposalEndpoint() {
        ...
        Long legacyId = new TransactionTemplate(transactions).execute(status -> {
            var deal = deals.findByProposalId(first).orElseThrow();
