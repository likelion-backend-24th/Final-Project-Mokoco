...
class AiDraftCacheTest {
    ...
    @Test void reusesResultWithoutQuotaAndReturnsIndependentCopiesUntilExpiry() {
        JsonNode first = cache.get("1:post:a", quota, this::generate);
        ((ObjectNode) first).put("requestId", "old");
    ...
    @Test void failedGenerationAllowsOnlyOneRetryAfterCooldown() {
        Supplier<JsonNode> failure = () -> {
            calls.incrementAndGet(); throw AiException.output();
        };
    ...
    @Test void concurrentDuplicateDoesNotCallProviderTwice() throws Exception {
        ...
        try (ExecutorService executor = Executors.newSingleThreadExecutor()) {
            Future<JsonNode> running = executor.submit(() -> cache.get("key", quota, () -> {
                entered.countDown();
                try {
