...
class NearbyRepairRequestQueryTest {
    ...
    @Test void filtersRegionVisibilityAndStatusBeforeCountingAndPaging() {
        PostCategory category = PostCategory.LIVING_ETC;
        Post first = post("A", true, PostStatus.WAITING, category);
        Post second = post("A", true, PostStatus.WAITING, category);
        ...
        Page<NearbyRepairRequest> page0 = repository.findNearby("A", null, PageRequest.of(0, 1, sort));
        Page<NearbyRepairRequest> page1 = repository.findNearby("A", null, PageRequest.of(1, 1, sort));

        assertThat(page0.getContent()).extracting(NearbyRepairRequest::id).containsExactly(second.getId());
        assertThat(page1.getContent()).extracting(NearbyRepairRequest::id).containsExactly(first.getId());
    ...
    @Test void categoryFilterAndDetailFields() {
        Post post = post("A", true, PostStatus.WAITING, PostCategory.LIVING_ETC);
        post("A", true, PostStatus.WAITING, PostCategory.PLUMBING);

        em.flush(); em.clear();

        Page<NearbyRepairRequest> result = repository.findNearby("A", PostCategory.LIVING_ETC, PageRequest.of(0, 20));
    ...
    @Test void broaderScopesIncludeOtherDongsAndDistrictsButNeverOtherProvinces() {
        Post sameDong = post("1165053100", true, PostStatus.WAITING, PostCategory.LIVING_ETC);
        Post sameDistrict = post("1165051000", true, PostStatus.WAITING, PostCategory.LIVING_ETC);
        Post sameProvince = post("1168051000", true, PostStatus.WAITING, PostCategory.LIVING_ETC);
        ...
        assertThat(repository.findNearby("11%", null, pageable).getContent()).extracting(NearbyRepairRequest::id)
                .containsExactly(sameProvince.getId(), sameDistrict.getId(), sameDong.getId());
        assertThat(repository.findNearby("11650%", null, pageable).getContent()).extracting(NearbyRepairRequest::id)
                .containsExactly(sameDistrict.getId(), sameDong.getId());
        assertThat(repository.findNearby("1165053100", null, pageable).getContent()).extracting(NearbyRepairRequest::id)
                .containsExactly(sameDong.getId());

        Page<NearbyRepairRequest> first = repository.findNearby("11%", null, PageRequest.of(0, 1, pageable.getSort()));
    ...
    @Test void guestQueryIncludesAllRegionsAndLegacyRequestsWithPagingAndCategoryFilter() {
        ...
        Post seoul = post("1165053100", true, PostStatus.WAITING, PostCategory.LIVING_ETC);
        Post busan = post("2611051000", true, PostStatus.WAITING, PostCategory.LIVING_ETC);
        Post legacy = post(null, true, PostStatus.WAITING, PostCategory.PLUMBING);
        ...
        Page<NearbyRepairRequest> first = repository.findNearby(null, null, PageRequest.of(0, 2, sort));

        assertThat(first.getTotalElements()).isEqualTo(3);
        assertThat(first.getContent()).extracting(NearbyRepairRequest::id).containsExactly(legacy.getId(), busan.getId());
        assertThat(repository.findNearby(null, null, PageRequest.of(1, 2, sort)).getContent())
                .extracting(NearbyRepairRequest::id).containsExactly(seoul.getId());
        assertThat(repository.findNearby(null, PostCategory.PLUMBING, PageRequest.of(0, 20)).getContent())
                .extracting(NearbyRepairRequest::id).containsExactly(legacy.getId());
    }
}
