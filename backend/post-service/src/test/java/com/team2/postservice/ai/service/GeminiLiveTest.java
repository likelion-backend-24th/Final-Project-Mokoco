...
class GeminiLiveTest {
    final GeminiClient client = new GeminiClient(
            new ObjectMapper(),System.getenv("GEMINI_API_KEY"),"gemini-flash-lite-latest",true
    );
    ...
    @Test void imageAndStructuredPostOutput() throws Exception {
        ...
        List<Map<String, Object>> parts = new AiImages().parts(List.of(new MockMultipartFile("images","blank.png","image/png",png.toByteArray())));

        parts.add(Map.of("text","이 사진에서 제품을 알 수 없으면 제품/모델을 null로 두세요. 제목은 '제품 확인 필요', 내용은 추가 사진 요청으로 작성하세요."));

        JsonNode result = client.generate(AiDraftService.COMMON, parts, AiDraftStatic.postSchema());
        AiDraftStatic.validatePost(result);
    }
    @Test void structuredContractOutputDoesNotInventMissingMoney() {

        Map<String, String> sources = Map.of("POST","의자 다리가 흔들려 수리를 요청합니다.");

        JsonNode result = client.generate(AiDraftService.COMMON + " 계약 초안: 모든 항목은 null로 두세요. fieldSources 각 항목은 sourceId=SUGGESTED_CLAUSE, quote=''로 두세요. 미합의 금액이나 날짜를 만들지 마세요.",
    ...
    @Test void productionContractPromptProducesGroundedDraft() {
        AiContractContext context = mock(AiContractContext.class);
