...
public class PaymentController {
    ...
    public record OrderResponse(String paymentId, Long postId, int baseAmount, int totalAmount, Long payeeId) {}
    @PostMapping("/prepare")
    public OrderResponse prepare(@RequestBody Prepare request, @AuthenticationPrincipal LoginUser user) {
        PaymentOrder order = paymentService.prepare(request.postId(), user.userId());
        return new OrderResponse(order.getPaymentId(), order.getPostId(), order.getBaseAmount(), order.getTotalAmount(), order.getPayeeId());
    }
    @PostMapping
