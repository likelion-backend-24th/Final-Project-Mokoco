...
export default function PostForm({
  ...
  const [content, setContent] = useState(() =>
      initialValue?.contentFormat === "HTML"
          ? initialValue.content
          : plainTextToHtml(initialValue?.content || ""),
  );

  const [contentText, setContentText] = useState(
      initialValue?.contentFormat === "PLAIN_TEXT"
          ? initialValue?.content || ""
          : "",
  );
  ...
  async function removeExistingImage(image) {
    ...
    try {
      const response = await fetch(
          backendUrl(
              `/api/posts/${postId}/images/${image.id}`,
          ),
          {
            method: "DELETE",
            headers: authHeaders,
            ...
            <div className="flex flex-wrap gap-2 mt-2">
              {selectedFiles.map(
                  (file, index) => (
                      <div
                          key={`${file.name}-${index}`}
                          className="relative bg-slate-100 px-3 py-1 rounded-lg text-xs flex items-center"
                      >
                        <span>{file.name}</span>
