...
export default function RegionScopeFilter({ regionScope = "ALL", regionFilter, category = "ALL", pathname = "/posts" }) {
  return (
    <nav aria-label="지역 조회 범위" className="region-scope-nav">
      ...
      <div className="region-scope-options">
        {REGION_SCOPES.map(({ value, label, field }) => (
          ...
          >
            <span className="region-scope-label">{label}{value === "ALL" ? " · 기본" : ""}</span>
            <span className="region-scope-value">
              {value === "ALL"
                ? "모든 지역"
                : regionFilter?.[field] ? `${regionFilter[field]}${value === "DONG" ? "" : " 전체"}` : `${label} 범위`}
            </span>
          </Link>
        ))}
      </div>
