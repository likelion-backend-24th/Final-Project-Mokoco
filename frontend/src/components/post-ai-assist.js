...
export default function PostAiAssist({ files, values, categories, onApply }) {
  ...
  async function analyze() {
    ...
    setBusy(true); setError(""); setResult(null); setUndo({});
    ...
    try {
      ...
      for (const field of ["title", "content", "category"]) {
        ...
        previous[field] = { before: current.values[field], applied: value };
        updated[field] = value;
        current.onApply(field, value);
      }
      setUndo(previous);
      setResult({ data: payload, skipped, inputKey: JSON.stringify([updated, files.map(f => [f.name, f.size, f.lastModified])]) });
  ...
  return <section className="ai-assist" aria-label="사진으로 작성 도움">
    ...
    {result && <div>
      ...
      <p>제품: {result.data.productType || "확인 필요"} · 모델명: {result.data.modelCandidate || "라벨 또는 직접 확인 필요"}</p>
      {result.inputKey !== inputKey && <p className="ai-warning">분석 후 입력이 바뀌었습니다. 아래는 이전 입력을 바탕으로 한 제안입니다.</p>}
      <ul>{result.data.observations.map((text, i) => <li key={i}>{text}</li>)}</ul>
      ...
      {result.data.questions.length > 0 && <><h3>추가로 알려주세요</h3><ul>{result.data.questions.map((text, i) => <li key={i}>{text}</li>)}</ul></>}
      <ul>{result.data.warnings.map((text, i) => <li key={i}>{text}</li>)}</ul>
