...
async function getAdminPayments(token) {
  ...
  try {
    const response = await fetch(backendUrl("/api/admin/payments?page=0&size=20"), {
...
export default async function AdminPaymentsPage() {
  ...
  const { data: payments, error } = await getAdminPayments(token);
  ...
  return (
    <section>
      ...
      <AdminPaymentTable initialPage={payments} />
    </section>
  );
}
