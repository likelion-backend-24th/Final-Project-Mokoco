    ...
    <section>
      ...
      <div className="dashboard-card">
        <div className="grid grid-cols-2 gap-3 sm:grid-cols-3">
          <StatCard icon={Users} label="총 회원 수" value={`${overview.totalUsers.toLocaleString("ko-KR")}명`} tone="blue" />
          <StatCard icon={UserPlus} label="오늘 가입자" value={`${overview.newUsersToday.toLocaleString("ko-KR")}명`} tone="blue" />
          <StatCard icon={Receipt} label="진행 중인 거래" value={`${overview.activeDeals.toLocaleString("ko-KR")}건`} tone="amber" />
          <StatCard icon={Flag} label="신고 대기" value={`${overview.pendingReports.toLocaleString("ko-KR")}건`} tone="amber" />
          <StatCard icon={CurrencyCircleDollar} label="거래 완료 금액" value={won(paymentSummary?.totalCompletedAmount)} tone="emerald" />
          <StatCard icon={HandCoins} label="정산된 금액" value={won(paymentSummary?.totalSettledAmount)} tone="emerald" />
        </div>
      </div>
    </section>
  );
