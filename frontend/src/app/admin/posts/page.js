...
async function getAdminPosts(token) {
  if (!token) return { data: null, error: "로그인이 필요합니다." };
  try {
    const response = await fetch(backendUrl("/api/admin/posts?page=0&size=20"), {
...
export default async function AdminPostsPage() {
  ...
  const { data: posts, error } = await getAdminPosts(token);
  ...
  return (
    <section>
      ...
      <AdminPostTable initialPage={posts} />
    </section>
  );
}
