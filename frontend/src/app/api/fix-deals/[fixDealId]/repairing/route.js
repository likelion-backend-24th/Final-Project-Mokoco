...
export async function PATCH(request, { params }) {
  ...
  try {
    const response = await fetch(backendUrl(`/fix-deals/${fixDealId}/repairing`), {
      method: "PATCH",
    ...
    if (!response.ok) {
      return Response.json({ error: errorMessage(payload, "수리 시작 처리에 실패했습니다."), code: payload?.code }, { status: response.status });
    }
