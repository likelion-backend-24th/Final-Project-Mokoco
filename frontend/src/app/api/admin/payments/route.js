...
// 관리자 결제/정산 상세 내역. status/page/size 쿼리를 그대로 백엔드에 전달한다.
export async function GET(request) {
  ...
  try {
    const response = await fetch(backendUrl(`/api/admin/payments?${query.toString()}`), {
    ...
    if (!response.ok) {
      return Response.json({ error: errorMessage(payload, "결제 목록을 불러오지 못했습니다.") }, { status: response.status });
    }
