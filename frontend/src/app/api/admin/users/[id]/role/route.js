...
export async function PATCH(request, { params }) {
  ...
  if (body?.role !== "ADMIN" && body?.role !== "USER") {
    return Response.json({ error: "role은 ADMIN 또는 USER여야 합니다." }, { status: 400 });
  }
  try {
    const response = await fetch(backendUrl(`/api/admin/users/${id}/role`), {
      method: "PATCH",
    ...
    if (!response.ok) {
      return Response.json({ error: errorMessage(payload, "권한 변경에 실패했습니다.") }, { status: response.status });
    }
